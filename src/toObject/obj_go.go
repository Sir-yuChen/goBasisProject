...
func main() {
	...
	*/
	...
	/*
		1. 方法值
			...
			选择器会返回一个方法"值"一个将方法(Point.Distance)绑定到特定接收器变量的函数。这个函数可以不通过指定其接收器即可被调用；
			即调用时不需要指定接收器，只要传入函数的参数即可
	*/
	p := Point{1, 2}
	q := Point{4, 6}
	// 方法值(相当于C语言的函数地址,函数指针)实际上distanceFormP 就绑定了 p接收器的方法Distance
	distanceFormP := p.Distance
	fmt.Println("方法值= distanceFormP ==》", distanceFormP(q))                 // "5"
	fmt.Println("方法值= distanceFormP 相当于 p对象直接调用方法函数传q参数==》", p.Distance(q)) // "5"

	//实际上distanceFormQ 就绑定了 q接收器的方法Distance
	distanceFormQ := q.Distance
	fmt.Println("方法值= distanceFormQ ==》", distanceFormQ(p))                 // "5"
	fmt.Println("方法值= distanceFormQ 相当于 q对象直接调用方法函数传p参数==》", q.Distance(p)) // "5"
	/*
		...
		举例来说，下面例子中的time.AfterFunc这个函数的功能是在指定的延迟时间之后来执行一个(译注：另外的)函数。且这个函数操作的是一个Rocket对象r
			type Rocket struct { msg string }
			func (r *Rocket) Launch() { fmt.Println("执行了Rocket的Launch函数") }
			r := new(Rocket)
	...
	*/
	...
	fmt.Println("方法表达式= distance1 ==》", distance1(p, q))
	fmt.Printf("%T\n", distance1) //%T表示打出数据类型 ,这个必须放在Printf使用

	distance2 := (*Point).Distance //方法表达式,必须传递指针类型
	fmt.Println("方法表达式= distance1 ==》", distance2(&p, q))
	fmt.Printf("%T\n", distance2)
	/*
		执行结果：
			方法表达式= distance1 ==》 5
...
// Path 定义一个Point切片类型 Path
type Path []Point

// TranslateBy 方法的接收器 是Path类型数据, 方法的选择器是TranslateBy(Point, bool)
