...
/*
	goroutine 协程并发
		...
		1. 与传统的系统级线程和进程相比，协程最大的优势在于“轻量级”。可以轻松创建上万个而不会导致系统资源衰竭。而线程和进程通常很难超过1万个。这也是协程别称“轻量级线程”的原因。
		2. 一个线程中可以有任意多个协程，但某一时刻只能有一个协程在运行，多个协程分享该线程分配到的计算机资源。
		3. 多数语言在语法层面并不直接支持协程，而是通过库的方式支持，但用库的方式支持的功能也并不完整，比如仅仅提供协程的创建、销毁与切换等能力。
			如果在这样的轻量级线程中调用一个同步 IO 操作，比如网络通信、本地文件读写，都会阻塞其他的并发执行轻量级线程，从而无法真正达到轻量级线程本身期望达到的目标。
		4. 在协程中，调用一个任务就像调用一个函数一样，消耗的系统资源最少！但能达到进程、线程并发相同的效果。
		5. 在一次并发任务中，进程、线程、协程均可以实现。从系统资源消耗的角度出发来看，进程相当多，线程次之，协程最少。
...
func newTask() {
	...
	for {
		i++
		second := int(time.Millisecond*1) * i
		fmt.Printf("new goroutine: i = %d  time= %d\n", i, second)
		time.Sleep(1 * time.Millisecond) //延时1s
		/*
			Goexit函数 线程退出
				调用 runtime.Goexit() 将立即终止当前 goroutine 执⾏，调度器确保所有已注册 defer 延迟调用被执行。
		*/
		if i == 5 {
			defer fmt.Println("new goroutine  defer  Goexit 之前")
			runtime.Goexit()               // 终止当前 goroutine, import "runtime"
			fmt.Println("Go线程 执行了Goexit ") // 不会执行
		}
	}
	//defer 如果在Goexit之后也不会被执行
	//defer fmt.Println("new goroutine  defer  Goexit 之后")
}

func main() {
	//创建一个 goroutine，启动另外一个任务
	defer fmt.Println("main goroutine  defer")
	go newTask()
	i := 0
	//main goroutine 循环打印
	for {
